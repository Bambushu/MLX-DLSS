...
> finishing pass, not a super-resolution miracle. The gain is real but modest; it does the most on soft,
> upscaled footage.
...
The fine-tuned re-detail here is **Neural Re-Detailer** (a.k.a. *DLSSDetailer*) — a detail-enhancement pass on top of the recovered renderer. The fine-tune weights are published at [`bambushu/neural-re-detailer`](https://huggingface.co/bambushu/neural-re-detailer) and **auto-download on first use** (no `--weights` needed); override the source with `MLXDLSS_HF_REPO`, set it to `""` to disable the fetch, or train your own with `scripts/finetune.py`. `--scale` upscales (Lanczos) before the re-detail pass; the re-detail synthesises mostly-new texture (only ~30% aligns with the true detail — enhancement, not reconstruction), so 1 is subtle, 2 is the visible default, 3 grains.
