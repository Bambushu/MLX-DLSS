...
def build_parser() -> argparse.ArgumentParser:
    ...
    run.add_argument("--scale", type=float, default=1.0, help="upscale factor (1.0-4.0) applied before re-detail via a Lanczos pre-pass (lossless FFV1 intermediate, no generation loss); 1.5-2 is the sweet spot, 1.0 = no upscale. Runs first, so --decode-args applies to the source; for bespoke pipelines, upscale separately and omit --scale")
    ...
    try:
        ...
        try:
            if args.scale != 1.0:
                ...
                source = scratch / "upscaled.mkv"
                # trunc to even dimensions for a clean scale; absolute input path so a leading-dash
                # filename can't be read as an ffmpeg option
                ...
                try:
                    subprocess.run([find_tool("ffmpeg", args.ffmpeg), "-hide_banner", "-loglevel", "error", "-y",
                                    "-i", str(Path(args.input).resolve()), "-map", "0:v:0", "-map", "0:a?", "-vf", vf,
                                    "-c:v", "ffv1", "-c:a", "copy", str(source)], check=True)  # lossless intermediate: no generation loss before re-detail
